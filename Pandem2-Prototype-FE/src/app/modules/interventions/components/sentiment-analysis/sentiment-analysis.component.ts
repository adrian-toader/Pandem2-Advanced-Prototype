...
import { SocialMediaAnalysisDataSplitType, SocialMediaAnalysisDataSubcategories, SocialMediaAnalysisSentimentType, SocialMediaAnalysisSentimentTypes } from '../../../../core/entities/socialMediaAnalysis-data.entity';
...
export class SentimentAnalysisComponent extends DashboardComponent implements OnInit, OnDestroy {
  ...
  public colorScheme = Constants.SOCIAL_MEDIA_ANALYSIS_SENTIMENT_COLORS;
  ...
  private prepareGraphData(): void {
    ...
    for (const type of sentimentTypes) {
      if (this.dailyChart?.split) {
        dailyChartSentimentData.push(this.dailyChart.split.find(item => item.name.toLowerCase() === type.toLowerCase()));
      }
    }
